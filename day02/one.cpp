...
bool repeats_twice(long long n){
	auto digits = int_digits(n);
	if (digits.size() % 2 == 1) return false; // uneven amount of digits doens't work
	return std::ranges::equal(
		digits | std::views::take(digits.size()/2),
		digits | std::views::drop(digits.size()/2) | std::views::take(digits.size()/2)
	);
}
